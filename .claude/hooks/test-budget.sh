...
FULL_SUITE_RE='^[[:space:]]*(npm|pnpm|yarn)[[:space:]]+(run[[:space:]]+)?test[[:space:]]*$|^[[:space:]]*pytest[[:space:]]*$|^[[:space:]]*go[[:space:]]+test[[:space:]]+\./\.\.\.[[:space:]]*$|^[[:space:]]*cargo[[:space:]]+test[[:space:]]*$|^[[:space:]]*(npx|bunx|pnpm([[:space:]]+(exec|dlx))?|yarn[[:space:]]+exec)[[:space:]]+(vitest|jest)([[:space:]]+(--run|run))?[[:space:]]*$|^[[:space:]]*(npm|pnpm|yarn)[[:space:]]+(run[[:space:]]+)?test:run[[:space:]]*$'
